...
# Скачиваем боевую модель из реестра по её Алиасу
...
dummy_data = pd.DataFrame(
    [[5.1, 3.5, 1.4, 0.2]],
    columns=[
        "sepal length (cm)",  # ← пробел перед (cm)
        "sepal width (cm)",  # ← пробел перед (cm)
        "petal length (cm)",  # ← пробел перед (cm)
        "petal width (cm)",  # ← пробел перед (cm)
    ],
)
